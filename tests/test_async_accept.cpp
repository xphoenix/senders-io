  ...
  sio::test::for_each_backend([]<class Backend>() {
    ...
    DYNAMIC_SECTION(std::string(Backend::name)) {
      ...
      if constexpr (sio::event_loop::socket_loop_for<loop_type, ip::tcp>) {
        ...
        sio::event_loop::acceptor<loop_type, ip::tcp> acceptor{
          &ctx, ip::tcp::v4(), ip::endpoint{ip::address_v4::any(), 2080}};
        stdexec::sender auto accept = let_value(
          acceptor.open(),
        ...
        sio::event_loop::socket<loop_type, ip::tcp> sock{&ctx, ip::tcp::v4()};
        stdexec::sender auto connect = let_value(
          sock.open(),
          [](auto&& client) {
        ...
        Backend::sync_wait(ctx, exec::when_any(accept, connect));
      } else {
        SUCCEED();
      }
    }
