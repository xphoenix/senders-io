...
namespace sio::event_loop::iouring {
  ...
  struct fd_read_submission_span {
    fd_read_submission_span(sio::mutable_buffer_span buffers, int fd, ::off_t offset) noexcept
      : buffers_{buffers}
      , fd_{fd}
      , offset_{offset} {
    }
  ...
  struct fd_read_submission_single {
    fd_read_submission_single(sio::mutable_buffer buffer, int fd, ::off_t offset) noexcept
      : buffer_{buffer}
      , fd_{fd}
      , offset_{offset} {
    }
  ...
  class fd_read_operation_base
    : public submission_operation<fd_read_operation_base<Submission, Receiver>, Receiver>
    , public Submission {
   public:
    fd_read_operation_base(
      io_context& ctx,
      auto data,
      int fd,
      ::off_t offset,
      ...
      : submission_operation<
          fd_read_operation_base<Submission, Receiver>,
  ...
  using fd_read_operation_span = fd_read_operation_base<fd_read_submission_span, Receiver>;

  template <class Receiver>
  using fd_read_operation_single = fd_read_operation_base<fd_read_submission_single, Receiver>;
  ...
  struct fd_read_factory {
    ...
    fd_read_sender_span operator()(sio::mutable_buffer_span data, ::off_t offset) const noexcept {
      return fd_read_sender_span{*context_, data, state_.native_handle(), offset};
    }

    fd_read_sender_single operator()(sio::mutable_buffer data, ::off_t offset) const noexcept {
      return fd_read_sender_single{*context_, data, state_.native_handle(), offset};
    }
  };
