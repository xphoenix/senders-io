...
namespace sio::event_loop::stdexec_backend {
  ...
  struct read_submission_single {
    ...
    ~read_submission_single() = default;
  ...
  using read_operation = stoppable_task_facade<read_operation_base<read_submission, Receiver>>;

  template <class Receiver>
  using read_operation_single =
    stoppable_task_facade<read_operation_base<read_submission_single, Receiver>>;
  ...
  struct read_sender {
    ...
    template <class Receiver>
    auto connect(Receiver rcvr) const -> read_operation<Receiver> {
      return {
        std::in_place,
        *context_,
        buffers_,
  ...
  struct read_sender_single {
    ...
    template <class Receiver>
    auto connect(Receiver rcvr) const -> read_operation_single<Receiver> {
      return {
        std::in_place,
        *context_,
        buffer_,
  ...
  struct read_factory {
    ...
    read_sender operator()(sio::mutable_buffer_span data, ::off_t offset) const noexcept {
      return read_sender{*context_, data, state_.native_handle(), offset};
    }

    read_sender_single operator()(sio::mutable_buffer data, ::off_t offset) const noexcept {
      return read_sender_single{*context_, data, state_.native_handle(), offset};
    }
  };
} // namespace sio::event_loop::stdexec_backend
