...
namespace sio::event_loop::epoll {
  namespace detail {
    ...
    class run_operation {
     ...
     public:
      run_operation(context& ctx, run_mode mode, Receiver&& rcvr) noexcept
        : context_{ctx}
        , mode_{mode}
  ...
  struct run_sender {
    ...
    auto connect(Receiver receiver) const {
      return detail::run_operation<Receiver>{*ctx_, mode_, static_cast<Receiver&&>(receiver)};
    }
  ...
  inline run_sender context::run(run_mode mode) noexcept {
